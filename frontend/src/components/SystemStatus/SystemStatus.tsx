    ...
    return (
        <div className="flex flex-col items-center justify-center space-y-6">
            <div className="system-card min-w-[360px] relative overflow-hidden">
                <div className="absolute top-0 right-0 w-8 h-8 border-t-2 border-r-2 border-main"></div>
                
                <div className="flex justify-between items-center mb-6 pb-2">
                    <span className="text-[9px] text-main font-bold uppercase tracking-[0.3em]">[ SYSTEM LINK ]</span>
                    <span className={`text-[9px] font-bold ${status.status === 'SYSTEM ACTIVE' ? 'text-good' : 'text-error animate-pulse'}`}>
                        {status.status}
                    </span>
                </div>
                <div className="divider-main" style={{ marginTop: '-1.5rem', marginBottom: '1.5rem' }} />
                ...
                <p className="font-console text-xs text-text-secondary text-center leading-relaxed tracking-tight px-4">
                    {">"} {status.message}
                </p>

                <div className="mt-8 relative px-4">
                    <div className="absolute -top-3 left-4 text-[8px] text-main font-bold uppercase">Initialization Progress</div>
                    <div className="h-[3px] w-full bg-neutral-black overflow-hidden">
