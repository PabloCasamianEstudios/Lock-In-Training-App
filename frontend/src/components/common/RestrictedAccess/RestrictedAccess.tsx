...
const RestrictedAccess: FC<RestrictedAccessProps> = ({ onLogout, title, message, buttonText }) => {
  const { t } = useLanguage();
  return (
  <div className="flex flex-col items-center justify-center h-[60vh] text-center space-y-6">
    ...
    <p className="text-xs font-black uppercase tracking-[0.3em] text-white/40 italic px-4 max-w-md">
      {message || t('restricted.login_prompt')}
    </p>
    <button
      onClick={onLogout}
    ...
    >
      {buttonText || t('restricted.login_button')}
    </button>
  </div>
  );
};
