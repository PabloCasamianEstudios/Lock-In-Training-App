        ...
        <div className="border border-white/10 bg-white/5 p-4 flex flex-col gap-3 flex-grow">
          ...
          <div className="grid grid-cols-2 gap-2">
            {profile?.stats && Object.entries(profile.stats).map(([k, v]) => (
              <div key={k} className="flex justify-between items-center text-xs p-2 bg-black/50 border border-white/5 hover:border-orange-500/30 transition-colors">
                <span className="text-white/50 uppercase font-mono">{k}</span>
                <span className="text-orange-400 font-black">{String(v)}</span>
              </div>
            ))}
            {!profile?.stats && (
              <div className="col-span-2 text-[10px] text-white/30 italic">Sin estadísticas disponibles</div>
            )}
          </div>
        </div>
      </div>
          ...
          {!session || (!session.isActive && session.hp <= 0) ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {session.pendingQuestId ? (
                <div className="col-span-1 md:col-span-2 text-center border border-red-500/50 bg-red-500/10 p-4 text-red-500 text-xs font-black uppercase tracking-widest animate-pulse">
