const formatError = (error: Error | string): string => {
    ...
    if (msg.includes("Invalid email format"))
        return "Formato de email no válido (falta @ o dominio).";
    ...
    if (msg.includes("Token expired"))
        return "Sesión caducada. Por favor, accede de nuevo.";

    return msg;
};

const BASE_URL = "https://lock-in-training-app.onrender.com"; // const BASE_URL = "http://localhost:8081";
...
const apiClient = async <T = any>(
    endpoint: string,
    { body, ...customConfig }: ApiClientConfig = {},
): Promise<T> => {
    ...
    try {
        console.log(
            `[API Request] Calling ${config.method || "GET"} ${url}...`,
        );
        ...
        try {
            ...
            console.warn(
                `[API] Failed to parse response from ${url}`,
                parseError,
            );
        ...
        if (!response.ok) {
            console.error(
                `[API Response Error] ${url} (Status: ${response.status})`,
                data,
            );
            const errorMessage =
                (typeof data === "object"
            ...
            if (
                ...
                console.warn(
                    "[API] Sesión caducada o acceso denegado. Cerrando sesión...",
                );
