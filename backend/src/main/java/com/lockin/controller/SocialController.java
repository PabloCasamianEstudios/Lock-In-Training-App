...
public class SocialController {
    ...
    public ResponseEntity<Object> sendRequest(@RequestParam Long senderId, @RequestParam Long receiverId) {
        if (senderId.equals(receiverId)) return ResponseEntity.badRequest().body("No puedes ser tu propio amigo");
        
        Optional<Friendship> existing = friendshipRepository.findExistingFriendship(senderId, receiverId);
        if (existing.isPresent()) return ResponseEntity.badRequest().body("Ya existe una relación o petición");

        User sender = userRepository.findById(senderId).orElse(null);
        User receiver = userRepository.findById(receiverId).orElse(null);
        if (sender == null || receiver == null) return ResponseEntity.status(404).body("Usuario no encontrado");
    ...
    public ResponseEntity<Object> acceptRequest(@PathVariable Long requestId) {
        ...
        if (friendship == null) return ResponseEntity.status(404).body("Petición no encontrada");
